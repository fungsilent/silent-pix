...
- `packages/event/src/server.ts` owns the generic type-keyed `EventServer`: callers inject the shared aggregate parser, and each connection record contains `key`, `clientId`, and `socket`. It validates envelopes before broadcast or point-to-point send; PHASE 1 still broadcasts every published event.
