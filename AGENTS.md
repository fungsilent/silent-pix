...
- Keep package ownership strict:
    ...
    - `packages/event`: generic browser WebSocket client and type-keyed `EventServer` transport helpers only; Elysia adapters remain in `apps/server`.
...
- Backend services own database membership, collation, `LIKE`, and related query semantics. Frontend cache code must not reimplement them; when response fields cannot decide membership exactly, invalidate and let the server refetch. Workflow list ordering is a precise exception in which Server and Web use the same `name` then `id` keys: Server declares SQLite ordering, while Web's realtime cache uses a file-local JavaScript ordinal comparator. Unicode edge ordering may differ between realtime upserts and a later refetch; this does not change membership, collation, or `LIKE` ownership for any other domain.
...
- Every event-producing synchronous Task/Workflow mutation declares `appApi.clientHeaders`, and its successful route publication excludes every socket matching `headers['client-id']`: Task create emits `task.created`, rename/flags emit `task.changed`, batch/single delete emit `task.removed`, Workflow create/update/archive emit `workflow.changed`, and true delete emits `workflow.removed`. Failure, validation, not-found, conflict, active-task, and empty-batch branches do not publish. The actor uses the REST response; remote clients use the event. Async task lifecycle and health events remain unfiltered broadcasts. Additional event types require a concrete domain need.
...
- `taskService` owns task resource queries and business operations, including startup recovery, snapshots, and public option methods. REST routes and `taskExecution` load canonical task snapshots and publish `task.changed` at their call sites; the Workflow domain owns the reused `toWorkflowSummary()` projection. `taskExecution` also owns generation orchestration and its generation-only completion transaction; `task.service.ts` must not import execution. This is a task-domain responsibility split, not a generic new layer.
- On server startup, `createApp()` awaits `taskService.failInterruptedTasks()` after `serverStore.init()` opens SQLite and before the first `ComfyClient.start()`. Persisted `queued` and `running` tasks are changed to `failed` with `errorCode = SERVER_RESTARTED`; startup does not resume them or publish recovery events because it runs before clients can connect, and clients obtain the recovered task state through subsequent REST initial/recovery synchronization. The create route launches `taskExecution.generate()` as a detached, untracked Promise; its `ComfyClient.execute(): void` call installs lifecycle callbacks whose output finalization is likewise untracked and not drained by graceful shutdown. Late finalization after the DB closes remains an unsupported known limit.
...
- ComfyUI execution lifecycle and normal outputs are driven by the backend `ComfyClient`'s ComfyUI WebSocket. `ComfyClient.execute()` is callback-driven and returns `void`: `onCompleted` and `onFailed` are required terminal callbacks, callbacks are invoked in lifecycle order without awaiting one another, and callback throw/rejection is logged without changing the Comfy outcome. REST history is only the recovery path after a pending prompt's socket disconnects; prompt submission, image download, and history cleanup remain REST. Do not place this ComfyUI protocol in `packages/event` or shared event contracts.
...
- `packages/event/src/server.ts` owns the generic type-keyed `EventServer`: callers inject the shared aggregate parser, and each connection record contains `key`, `clientId`, and `socket`. It validates envelopes once before publication; `publish()` can exclude every matching `clientId` record while `send()` remains point-to-point.
- `apps/server/src/module/event/event.route.ts` owns the Elysia `/api/event` adapter, same-origin handshake, query validation, and open/close lifecycle hooks; the Workflow domain owns the reused `toWorkflowSummary()` projection, while health composition supplies initial-send and timer hooks.
- The server validates every outbound event through the shared aggregate schema before broadcast.
- Client identity is the per-page UUID `clientId` exported by the Web API client module (`apps/web/src/api/api.client.ts`); its shared fetcher supplies it as `client-id`, while the WebSocket query imports the same value. `appApi.clientHeaders` is the single shared REST header schema; mutation routes validate it before handlers run and use it only as EventServer routing metadata. All synchronous Task/Workflow publications exclude matching sockets; async lifecycle and health events remain broadcasts.
...
- `apps/web/src/features/event/event.client.ts` owns the page-scoped WebSocket URL/client identity, browser lifecycle, inbound aggregate parsing, connection status, reconnect recovery, and exhaustive dispatch; domain cache handlers remain under their existing feature folders. `App.tsx` only starts and cleans up this feature.
...
- Task generation downloads Comfy outputs outside the image lock; `taskExecution` holds `withImageMutation` only across ingest, reference commit, and orphan cleanup, then publishes the committed snapshot before successful Comfy output unlink.
