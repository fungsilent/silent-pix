...
- Task create is one request. The shared contract must reject providing both a stored reference image id and an uploaded file; neither means txt2img. The current contract still lacks this mutual-exclusion check; preserve the requirement when fixing it.
