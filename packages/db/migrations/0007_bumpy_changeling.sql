...
CREATE TABLE `__new_tasks` (
	...
	`discard` integer DEFAULT false NOT NULL,
	...
	CONSTRAINT "tasks_status_check" CHECK("__new_tasks"."status" in ('queued', 'running', 'done', 'failed')),
	CONSTRAINT "tasks_flag_exclusive_check" CHECK(not ("__new_tasks"."pin" = 1 and "__new_tasks"."discard" = 1))
);
--> statement-breakpoint
INSERT INTO `__new_tasks`("id", "name", "status", "pin", "discard", "workflow_id", "config", "workflow_revision", "created_at", "updated_at", "comfy_prompt_id", "error_code", "error_message") SELECT "id", "name", "status", "pin", "discard", "workflow_id", "config", "workflow_revision", "created_at", "updated_at", "comfy_prompt_id", "error_code", "error_message" FROM `tasks`;--> statement-breakpoint
